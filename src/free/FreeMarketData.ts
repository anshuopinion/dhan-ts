...
export class FreeMarketData {
  ...
  async getFreeIntradayHistoricalData(
    ...
    try {
      ...
      const response = await axios.post<DhanApiResponse>(
        ...
        dhanRequest,
        {
          headers: this.getApiHeaders(request.webAccess),
        }
      );
  ...
  async getFreeHistoricalData(
    ...
    if (!request.symbol) {
      throw new Error(
        "Symbol is required for daily/weekly/monthly data. Please provide the symbol parameter."
      );
    }
  ...
  async getStockBasicDetails(
    ...
    try {
      ...
      const response = await axios.post<{
        code: number;
        data: StockBasicDetailsResponse;
        remarks: string;
      }>(`${this.SCANX_API_URL}/scanx/rtscrdt`, requestPayload, {
        headers: {
      ...
      if (response.data.code !== 0) {
        throw new Error(
          `ScanX API Error: ${response.data.remarks || "Unknown error"}`
        );
      }
  ...
  async getStockFundamentals(
    ...
    try {
      ...
      throw new Error(
        `Failed to fetch stock fundamental data: ${error.message}`
      );
    }
  }
