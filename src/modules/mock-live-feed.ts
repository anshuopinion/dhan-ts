...
export class MockLiveFeed extends EventEmitter {
	...
	subscribe(instruments: Instrument[], requestCode: FeedRequestCode): void {
		...
		instruments.forEach(([_, securityId]) => {
			if (!this.securityConfigs.has(securityId)) {
				console.warn(`No price configuration found for security ${securityId}. Using default values.`);
	...
	private isWithinMarketHours(): boolean {
		...
		return currentTime >= this.marketHours.start && currentTime <= this.marketHours.end;
	}
	...
	private generateTickerResponse(exchangeSegment: ExchangeSegment, securityId: string): TickerResponse {
	...
	private generateQuoteResponse(exchangeSegment: ExchangeSegment, securityId: string): QuoteResponse {
		const price = this.generatePrice(securityId);
		const config = this.securityConfigs.get(securityId)!;
	...
	private generateOiDataResponse(exchangeSegment: ExchangeSegment, securityId: number): OiDataResponse {
		return {
			type: "oi_data",
			exchangeSegment,
			securityId,
	...
	private generatePrevCloseResponse(exchangeSegment: ExchangeSegment, securityId: number, price: number): PrevCloseResponse {
		return {
			type: "prev_close",
			exchangeSegment,
			securityId,
	...
	private generateFullResponse(exchangeSegment: ExchangeSegment, securityId: number, price: number): FullMarketDataResponse {
		return {
			type: "full",
			exchangeSegment,
			securityId,
			lastTradedPrice: price,
			lastTradedQuantity: Math.floor(Math.random() * 100) + 1,
			lastTradedTime: new Date().getTime(),
					...
					instruments.forEach(([exchangeSegment, securityId]) => {
						...
						switch (requestCode) {
							case FeedRequestCode.SUBSCRIBE_TICKER:
								this.emit("data", this.generateTickerResponse(exchangeSegment, securityId));
								break;
							case FeedRequestCode.SUBSCRIBE_QUOTE:
								this.emit("data", this.generateQuoteResponse(exchangeSegment, securityId));
								break;

							case FeedRequestCode.SUBSCRIBE_FULL:
								this.emit("data", this.generateFullResponse(exchangeSegment, Number(securityId), this.generatePrice(securityId)));
								break;
							default:
								console.warn(`Unknown request code ${requestCode}. Skipping data generation.`);
						}
					});
				});
			}
	...
	unsubscribe(instruments: Instrument[]): void {
		...
		this.subscribedInstruments.forEach((storedInstruments, requestCode) => {
			const remainingInstruments = storedInstruments.filter(inst => !instruments.some(unsubInst => inst[0] === unsubInst[0] && inst[1] === unsubInst[1]));
