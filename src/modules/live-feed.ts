...
export class LiveFeed {
	...
	private getReconnectDelay(): number {
		...
		const exponentialDelay = Math.min(this.baseReconnectDelay * Math.pow(2, this.reconnectAttempts), this.maxReconnectDelay);
	...
	private async attemptReconnect() {
		if (this.isIntentionalClose) {
			console.log("Connection was intentionally closed, not attempting reconnect");
			return;
		}
	...
	private async resubscribeAll() {
		...
		for (const [requestCode, instruments] of subscriptions) {
			try {
				...
				console.log(`Resubscribed to ${instruments.length} instruments with request code ${requestCode}`);
			} catch (error) {
				console.error(`Failed to resubscribe to instruments with request code ${requestCode}:`, error);
			...
			try {
				...
				this.ws.on("pong", () => {
					...
					if (this.ws) {
						...
						this.ws.once("timeout", () => this.handleConnectionError(new Error("Connection timeout")));
					}
				});
			} catch (error) {
				this.connectionState = "disconnected";
	...
	private listeners: {
		[event: string]: ((data: LiveFeedResponse | DisconnectionResponse | Error | {code: number; reason: string}) => void)[];
	} = {};

	on(event: "data", listener: (data: LiveFeedResponse) => void): void;
	on(event: "disconnected", listener: (data: DisconnectionResponse) => void): void;
	on(event: "error", listener: (error: Error) => void): void;
	on(event: "close", listener: (data: {code: number; reason: string}) => void): void;
	...
	private emit(event: string, data: LiveFeedResponse | DisconnectionResponse | Error | {code: number; reason: string}): void {
