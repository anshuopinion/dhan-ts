...
export class MarketData {
  ...
  async getLTP(request: MarketFeedRequest): Promise<MarketLTPResponse> {
    const response = await this.axiosInstance.post<MarketLTPResponse>(
      "/v2/marketfeed/ltp",
      request
    );
  ...
  async getOHLC(request: MarketFeedRequest): Promise<MarketOHLCResponse> {
    const response = await this.axiosInstance.post<MarketOHLCResponse>(
      "/v2/marketfeed/ohlc",
      request
    );
    return response.data;
  ...
  async getQuote(request: MarketFeedRequest): Promise<MarketQuoteResponse> {
    const response = await this.axiosInstance.post<MarketQuoteResponse>(
      "/v2/marketfeed/quote",
      request
    );
    return response.data;
  ...
  private combineIntradayCandles(
    ...
    intervalInfo: { baseInterval: number; multiplier: number }
  ): HistoricalDataResponse {
    ...
    for (let i = 0; i < data.timestamp.length; i++) {
      ...
      const minutesSinceMarketOpen = Math.floor(
        (currentTimestamp - marketOpenTime) / 60
      );
