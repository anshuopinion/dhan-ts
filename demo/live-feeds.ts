...
export async function demoFourLiveFeeds(config: DhanConfig) {
  console.log("\nDemonstrating Four Simultaneous LiveFeed Connections:");

  const specs: { tag: string; instrument: Instrument }[] = [
    { tag: "A", instrument: [ExchangeSegment.NSE_EQ, "14043"] }, // Reliance
    { tag: "B", instrument: [ExchangeSegment.NSE_EQ, "11536"] }, // HDFC Bank
    { tag: "C", instrument: [ExchangeSegment.NSE_EQ, "9931"] }, // ICICI Bank
    { tag: "CE", instrument: [ExchangeSegment.NSE_EQ, "9931"] }, // ICICI Bank
    { tag: "D", instrument: [ExchangeSegment.NSE_EQ, "7508"] }, // SBI
  ];

  const feeds = specs.map((s) => ({
    tag: s.tag,
  ...
  // Bind handlers BEFORE connect so we capture all events
  for (const f of feeds) {
    f.feed.liveFeed.on("data", (data) => console.log(`[${f.tag}] data:`, data));
    f.feed.liveFeed.on("error", (err) =>
      console.error(`[${f.tag}] error:`, err.message ?? err),
    );
    f.feed.liveFeed.on("close", (e) => console.log(`[${f.tag}] closed:`, e));
  }

  // Connect + subscribe sequentially (Dhan rejects rapid handshakes with 429;
  // also subscribe must follow connect immediately or ws idle-closes 1006)
  for (const f of feeds) {
    try {
      await f.feed.liveFeed.connect();
      console.log(`[${f.tag}] connected`);
      f.feed.liveFeed.subscribe(
        [f.instrument],
        FeedRequestCode.SUBSCRIBE_QUOTE,
      );
      console.log(`[${f.tag}] subscribed sec ${f.instrument[1]}`);
    } catch (err: any) {
      console.error(`[${f.tag}] connect/subscribe failed:`, err.message ?? err);
    }
  ...
  process.on("SIGINT", () => {
    ...
    feeds.forEach((f) => f.feed.liveFeed.close());
    process.exit(0);
  });
}
...
export async function demoMultiConnectionLiveFeed(dhanFeed: DhanFeed) {
  ...
  try {
    ...
    dhanFeed.multiConnectionLiveFeed.on("message", ({ connectionId, data }) => {
      ...
      switch (data.type) {
        case "ticker":
          ...
          console.log(
            `Last Traded Time: ${data.lastTradedTime ? new Date(data.lastTradedTime * 1000).toLocaleTimeString() : "N/A"}`,
          );
          break;
        ...
        case "full":
          ...
          if (data.marketDepth) {
            ...
            data.marketDepth.buy.forEach((level: any, index: number) => {
              console.log(
                `  Level ${index + 1}: Price ₹${level.price}, Qty ${level.quantity}, Orders ${level.orders}`,
              );
            });
    ...
    dhanFeed.multiConnectionLiveFeed.on("data", (data) => {
      const timestamp = new Date().toLocaleTimeString();
      console.log(
        `[${timestamp}] 📡 Data Event: ${data.type} for Security ${data.securityId}`,
      );
    });
    ...
    dhanFeed.multiConnectionLiveFeed.on("close", (event) => {
      console.log(
        `Connection ${event.connectionId} closed: ${event.code} - ${event.reason}`,
      );
    });
    ...
    await dhanFeed.multiConnectionLiveFeed.subscribe(
      instrumentSet1,
      FeedRequestCode.SUBSCRIBE_QUOTE,
    );
    ...
    const connectionStatus =
      dhanFeed.multiConnectionLiveFeed.getConnectionStatus();
    console.log("\n🔗 Connection Status:");
    connectionStatus.forEach((conn) => {
      console.log(
        `  Connection ${conn.connectionId}: ${conn.state} (${conn.instrumentCount} instruments)`,
      );
    });
...
export async function demoMockMultiConnectionLiveFeed(dhanFeed: DhanFeed) {
  ...
  try {
    ...
    dhanFeed.mockMultiConnectionLiveFeed.on("data", (data) => {
      const timestamp = new Date().toLocaleTimeString();
      console.log(
        `[${timestamp}] 🎭 Mock Connection ${data.connectionId} - Received data:`,
        {
          type: data.type,
    ...
    dhanFeed.mockMultiConnectionLiveFeed.on("error", (error) => {
      console.error(
        `🎭 Mock Connection ${error.connectionId} error:`,
    ...
    dhanFeed.mockMultiConnectionLiveFeed.on("close", (event) => {
      console.log(
        `🎭 Mock Connection ${event.connectionId} closed: ${event.code} - ${event.reason}`,
      );
    });

    // Subscribe to different instrument sets
    console.log("🎭 Subscribing to first set of mock instruments (TICKER)...");
    await dhanFeed.mockMultiConnectionLiveFeed.subscribe(
      instrumentSet1,
      FeedRequestCode.SUBSCRIBE_TICKER,
    );
    ...
    // Show connection status
    const connectionStatus =
      dhanFeed.mockMultiConnectionLiveFeed.getConnectionStatus();
    ...
    console.log(
      "🎭 This is a simulated feed that generates realistic market data",
    );
    console.log(
      "💡 Perfect for testing multi-connection logic without real market data",
    );
