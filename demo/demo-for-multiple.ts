...
class MultiConnectionDemo {
	...
	private handleMarketData(connectionId: number, data: LiveFeedResponse) {
		...
		if ('type' in data && data.type === "ticker") {
			...
			if (this.messageStats.ticker % 100 === 0) {
				console.log(`📈 Ticker #${this.messageStats.ticker}: ${data.exchangeSegment}:${data.securityId} @ ₹${data.lastTradedPrice}`);
			}
		} else if ('type' in data && data.type === "quote") {
			this.messageStats.quote++;
			if (this.messageStats.quote % 50 === 0) {
				console.log(`📊 Quote #${this.messageStats.quote}: ${data.exchangeSegment}:${data.securityId} | Price: ₹${data.lastTradedPrice} | Volume: ${data.volumeTraded}`);
			}
		} else if ('type' in data && data.type === "full") {
			this.messageStats.full++;
			if (this.messageStats.full % 25 === 0) {
				console.log(`📋 Full Market Data #${this.messageStats.full}: ${data.exchangeSegment}:${data.securityId} | OHLC: ${data.openPrice}/${data.highPrice}/${data.lowPrice}/${data.closePrice}`);
			}
	...
	private getConnectionInfo(): string {
		...
		return connections.map(conn => 
			`${conn.connectionId}(${conn.state}:${conn.instrumentCount})`
		).join(", ");
	}
