...
async function demoMarketData() {
	...
	console.log("📋 Note: Market Data APIs have a rate limit of 1 request per second");
	console.log("📋 Up to 1000 instruments can be fetched in a single API request");
	...
	try {
		...
		Object.entries(quote.data).forEach(([exchangeSegment, instruments]) => {
			...
			Object.entries(instruments).forEach(([securityId, data]) => {
				...
				console.log(`    Circuit Limits: ₹${data.lower_circuit_limit} - ₹${data.upper_circuit_limit}`);
				console.log(`    Net Change: ₹${data.net_change}`);
				
				// OHLC within quote
				console.log(`    OHLC: O:₹${data.ohlc.open} H:₹${data.ohlc.high} L:₹${data.ohlc.low} C:₹${data.ohlc.close}`);
				
				// Open Interest (for derivatives)
				if (data.oi > 0) {
					console.log(`    Open Interest: ${data.oi} (High: ${data.oi_day_high}, Low: ${data.oi_day_low})`);
				}
		...
		if (error.response) {
			...
			if (status === 429) {
				console.error("💡 Rate Limit Hit: The API allows only 1 request per second for market data endpoints");
			} else if (status === 401) {
				console.error("💡 Authentication Failed: Please check your access token and client ID in .env file");
			} else if (status === 400) {
				console.error("💡 Bad Request: Check the request parameters, security IDs, and date formats");
			} else if (data?.data?.['805']) {
				console.error("💡 Rate Limiting: Too many requests. Wait before making more requests.");
			}
		}
	}
