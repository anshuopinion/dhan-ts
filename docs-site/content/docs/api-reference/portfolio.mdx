...
Exit all open positions at market price and cancel all open orders for the current trading day.
...
async function exitEverything() {
  try {
    await client.portfolio.exitAllPositions();
    console.log('All positions exited and open orders cancelled');
  } catch (error) {
...
<Callout type="warn">
  This is irreversible — it squares off **every** open position at market price and cancels all open orders. The API returns `202 Accepted` with no response body. Requires Static IP whitelisting.
</Callout>
