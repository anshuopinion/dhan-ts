...
Calculate the combined margin for multiple orders in a single request, including hedge benefits across the basket (span, exposure, equity, F&O, commodity, currency).
...
<Callout type="info">
  Margin values returned are indicative and valid only for the current trading session. Use `includePosition` / `includeOrder` to factor existing positions and open orders into the hedge calculation.
</Callout>
...
async function checkBasketMargin() {
  try {
    const margin = await client.funds.calculateMultiOrderMargin({
      dhanClientId: 'your-client-id',
    ...
    console.log('Total Margin (with hedge benefit):', margin.totalMargin);
    console.log('SPAN Margin:', margin.spanMargin);
    console.log('Exposure:', margin.exposure);
    console.log('F&O Margin:', margin.foMargin);
  } catch (error) {
    console.error('Error calculating multi-order margin:', error);
  }
}
```
...
```typescript
interface MultiOrderMarginScrip {
  exchangeSegment: ExchangeSegment;
  transactionType: TransactionType;
  quantity: number;
  productType: ProductType;
...
interface MultiOrderMarginRequest {
  dhanClientId: string;
  includePosition?: boolean; // factor existing positions into hedge calc
  includeOrder?: boolean;    // factor open orders into hedge calc
...
```typescript
interface MultiOrderMarginResponse {
  clientId?: string;
  totalMargin?: number;   // overall combined margin (with hedge benefit)
  spanMargin?: number;
  exposure?: number;
  equityMargin?: number;
  foMargin?: number;
