...
Renew a currently-active Dhan Web access token for another 24 hours. Returns a fresh token with new expiry. The `dhanClientId` is sent as a request header.
...
async function renewAccessToken() {
  try {
    const renewed = await client.authentication.renewToken('your-client-id');

    console.log('New Access Token:', renewed.accessToken);
    console.log('Expires At:', renewed.expiryTime);
    console.log('Client Name:', renewed.dhanClientName);
    console.log('UCC:', renewed.dhanClientUcc);
  } catch (error) {
...
<Callout type="warn">
  Only **active** tokens can be renewed — renewing an already-expired token returns an error. Renew before expiry to keep a session alive.
</Callout>
...
interface RenewTokenResponse {
  ...
  accessToken: string;          // newly generated token
  expiryTime: string;           // new expiry timestamp
}
```
